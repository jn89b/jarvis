...
class BaseDataset(Dataset):
    ...
    def load_data(self):
        ...
        for cnt, data_path in enumerate(self.data_path):
            ...
            self.cache_path = os.path.join(
                data_path, f'cache_{self.config.method.model_name}')
            ...
            if self.config['use_cache'] or is_ddp():
                ...
                if os.path.exists(self.cache_path) and self.config.get('overwrite_cache', False) is False:
                    print('Warning: cache path {} already exists, skip '.format(
                        self.cache_path))
                    file_list = self.get_data_list(data_usage_this_dataset)
                else:
                    ...
                    data_splits = [(data_path, mapping, list(
                        data_splits[i]), dataset_name) for i in range(process_num)]
            ...
            print('Loaded {} samples from {}'.format(
                len(file_list) * self.data_chunk_size, data_path))
            ...
            if self.config['store_data_in_memory']:
                ...
                print('Loaded {} data into memory'.format(
                    len(self.data_loaded_memory)))
        # if not self.is_validation:
    ...
    def preprocess(self, scenario):
        ...
        for k, v in map_feat.items():
            ...
            cur_info['polyline_index'] = (
                point_cnt, point_cnt + len(cur_polyline))
        ...
        ret['map_center'] = scenario['metadata'].get(
            'map_center', np.zeros(3))[np.newaxis]
    ...
    def process(self, internal_format):
        ...
        if info['map_infos']['all_polylines'].__len__() == 0:
            info['map_infos']['all_polylines'] = np.zeros(
        ...
        ret_dict['map_center'] = ret_dict['map_center'].repeat(
            sample_num, axis=0)
    ...
    def get_map_data(self, center_objects, map_infos):
        ...
        map_polylines = transform_to_center_coordinates(
            neighboring_polylines=polylines)
        ...
        for k, v in map_infos.items():
            ...
            for polyline_dict in v:
                ...
                polyline_segment_x = polyline_segment[:,
                                                      :, 0] - center_offset[0]
                polyline_segment_y = polyline_segment[:,
                                                      :, 1] - center_offset[1]
                ...
                for i in range(polyline_segment.shape[0]):
                    segment_index_list.append(
                        find_true_segments(in_range_mask[i]))
        ...
        polyline_xy_offsetted = batch_polylines[:, :, :,
                                                0:2] - np.reshape(center_offset, (1, 1, 1, 2))
        ...
        topk_idxs = np.argsort(polyline_center_dist,
                               axis=-1)[:, :num_of_src_polylines]
        ...
        map_polylines = np.concatenate(
            (map_polylines, xy_pos_pre, map_types), axis=-1)
    ...
    def get_manually_split_map_data(self, center_objects, map_infos):
        ...
        def transform_to_center_coordinates(neighboring_polylines, neighboring_polyline_valid_mask):
            neighboring_polylines[:, :, :,
                                  0:3] -= center_objects[:, None, None, 0:3]
            neighboring_polylines[:, :, :, 0:2] = common_utils.rotate_points_along_z(
        ...
        def append_single_polyline(new_polyline):
            ...
            cur_valid_mask = np.zeros(
                (num_points_each_polyline), dtype=np.int32)
        ...
        if len(batch_polylines) > num_of_src_polylines:
            ...
            center_offset_rot = np.tile(np.array(center_offset, dtype=np.float32)[
                                        None, :], (num_center_objects, 1))
            ...
            pos_of_map_centers = center_objects[:,
                                                0:2] + center_offset_rot[:, 0]
    ...
    def sample_from_distribution(self, original_array, m=100):
        ...
        bins = np.array([float(range_.split(',')[1])
                        for range_, _ in distribution])
        sample_sizes = np.array([round(perc / 100 * m)
                                for _, perc in distribution])
