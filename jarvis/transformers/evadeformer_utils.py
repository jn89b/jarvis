...
class RotaryPositionEmbedding:
    ...
    def rotate(self, t):
        ...
        t_rot = (t_rot * pos_enc.cos()) + \
            (self._rotate_half(t_rot) * pos_enc.sin())
...
class Residual(nn.Module):
    ...
    def forward(self, *args, **kwargs):
        ...
        output.last_hidden_state = self.dropout(
            output.last_hidden_state) + args[0]
        return output
...
class MultiHeadAttention(nn.Module):
    def __init__(
        ...
        self.q_proj = nn.Linear(num_q_input_channels,
                                num_qk_channels, bias=qkv_bias)
        self.k_proj = nn.Linear(num_kv_input_channels,
                                num_qk_channels, bias=qkv_bias)
        self.v_proj = nn.Linear(num_kv_input_channels,
                                num_v_channels, bias=qkv_bias)
    ...
    def forward(
        ...
        if self.causal_attention:
            ...
            causal_mask = torch.ones(
                (i, j), device=x_q.device, dtype=torch.bool).triu(j - i + 1)
        ...
        for q_chunk, k_chunk, v_chunk in zip(
            ...
            attn = torch.einsum(
                "b h i c, b h j c -> b h i j", q_chunk, k_chunk)
...
class CrossAttentionLayer(AbstractAttentionLayer):
    def __init__(
        ...
        super().__init__(
            Residual(
                cross_attn, residual_dropout) if attention_residual else cross_attn,
            Residual(MLP(num_q_input_channels, widening_factor,
                     bias=mlp_bias), residual_dropout),
        )
    ...
    def forward(
        ...
        for i, layer in enumerate(self):
            ...
            output = layer(x, pad_mask=pad_mask,
                           rot_pos_emb=rot_pos_emb_i, kv_cache=kv_cache_i)
...
class PerceiverEncoder(nn.Module):
    def __init__(
        ...
        self.latent_provider = TrainableQueryProvider(
            num_latents, num_latent_channels, init_scale=0.02)
