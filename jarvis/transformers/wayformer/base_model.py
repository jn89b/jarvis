...
class BaseModel(pl.LightningModule):
    ...
    def init_nuscenes(self):
        if self.config.get('eval_nuscenes', False):
            ...
            nusc = NuScenes(version='v1.0-trainval',
                            dataroot=self.config['nuscenes_dataroot'])
            ...
            self.pred_config5 = PredictionConfig.deserialize(
                pred_config, self.helper)
    ...
    def compute_metrics_nuscenes(self, pred_dicts):
        ...
        metric_results = compute_metrics(
            pred_dicts, self.helper, self.pred_config5)
    ...
    def compute_metrics_av2(self, pred_dicts):
        ...
        metric_results = argoverse2_evaluation(pred_dicts=pred_dicts,
                                               num_modes_for_eval=num_modes_for_eval)
        self.log('val/av2_official_minADE6',
                 metric_results['min_ADE'], prog_bar=True, on_step=False, on_epoch=True)
        self.log('val/av2_official_minFDE6',
                 metric_results['min_FDE'], prog_bar=True, on_step=False, on_epoch=True)
        self.log('val/av2_official_brier_minADE',
                 metric_results['brier_min_ADE'], prog_bar=True, on_step=False, on_epoch=True)
        self.log('val/av2_official_brier_minFDE',
                 metric_results['brier_min_FDE'], prog_bar=True, on_step=False, on_epoch=True)
        self.log('val/av2_official_miss_rate',
                 metric_results['miss_rate'], prog_bar=True, on_step=False, on_epoch=True)
    ...
    def log_info(self, batch, batch_idx, prediction, status='train'):
        ...
        ade_losses = torch.sum(ade_diff * gt_traj_mask,
                               dim=-1) / torch.sum(gt_traj_mask, dim=-1)
        ...
        center_gt_final_valid_idx = center_gt_final_valid_idx.view(
            -1, 1, 1).repeat(1, modes, 1).to(torch.int64)

        fde = torch.gather(
            ade_diff, -1, center_gt_final_valid_idx).cpu().detach().numpy().squeeze(-1)
        ...
        for dataset_name in unique_dataset_names:
            batch_idx_for_this_dataset = np.argwhere(
                [n == str(dataset_name) for n in dataset_names])[:, 0]
            for key in loss_dict.keys():
                new_dict[dataset_name + '/' +
                         key] = loss_dict[key][batch_idx_for_this_dataset]
        ...
        if status == 'val' and self.config.get('eval', False):
            ...
            for traj_type in range(8):
                batch_idx_for_traj_type = np.where(
                    trajectory_types == traj_type)[0]
            ...
            for kalman_bucket, (low, high) in {"easy": [0, 30], "medium": [30, 60], "hard": [60, 9999999]}.items():
                batch_idx_for_kalman_diff = \
                    np.where(np.logical_and(low <= kalman_difficulties,
                             kalman_difficulties < high))[0]
                if len(batch_idx_for_kalman_diff) > 0:
                    for key in important_metrics:
                        new_dict["kalman/" + kalman_bucket + "_" +
                                 key] = loss_dict[key][batch_idx_for_kalman_diff]
            loss_dict.update(new_dict)
            ...
            for type in agent_types:
                batch_idx_for_type = np.where(
                    inputs['center_objects_type'] == type)[0]
        ...
        for k, v in loss_dict.items():
            self.log(status + "/" + k, v, on_step=False, on_epoch=True,
                     sync_dist=True, batch_size=size_dict[k])
