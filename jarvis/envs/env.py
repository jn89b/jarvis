...
class DynamicThreatAvoidance(AbstractBattleEnv):
    ...
    def __init__agents(self) -> None:
        ...
        for i in range(num_evaders):
            ...
            rand_velocity = np.random.uniform(
                self.state_limits['v']['min'], self.state_limits['v']['max'])
        ...
        for i in range(num_pursuers):
            ...
            agent_id += 1
        self.battlespace.all_agents: List[Agent] = self.all_agents
        self.agents = [
    ...
    def __init_observation_spaces(self) -> spaces.Dict:
        ...
        for agent in self.agents:
            agent: Agent = agent
            # obs_spaces[agent.id] = self.get_agent_observation_space(agent.id)
            return self.get_agent_observation_space(agent.id)
    ...
    def get_current_observation(self, agent_id: int) -> np.ndarray:
        ...
        for other_agent in self.battlespace.all_agents:
            ...
            # clip distance
            rel_distance = np.clip(rel_distance, self.config.low_rel_pos,
                                   self.config.high_rel_pos)
            rel_velocity = np.clip(rel_velocity, self.config.low_rel_vel,
                                   self.config.high_rel_vel)
            rel_heading = np.clip(rel_heading, self.config.low_rel_att,
    ...
    def step(self, action: np.ndarray,
        ...
        obs = self.get_current_observation(self.vehicle.id)
