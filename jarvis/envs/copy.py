...
class BattleEnv(gymnasium.Env):
    ...
    def __init__(self,
        ...
        self.obs_mean = np.zeros(self.init_observation_spaces().shape, dtype=np.float32)
        self.obs_var = np.ones(self.init_observation_spaces().shape, dtype=np.float32)
        self.obs_count = 1e-4  # Small count to avoid division by zero
    ...
    def normalize_observation(self, observation):
        ...
        obs_std = np.sqrt(self.obs_var / (self.obs_count - 1))
        normalized_obs = (observation - self.obs_mean) / (obs_std + 1e-8)
        return np.clip(normalized_obs, -5, 5)  # Clip to avoid extreme values
    ...
    def init_action_spaces(self, use_stable_baselines: bool) -> spaces.Dict:
        ...
        for agent in self.battlespace.agents:
            ...
            if agent.is_controlled:
                ...
                action_space = self.get_agent_action_space(agent_id=agent.id)
