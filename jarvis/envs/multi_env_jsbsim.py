...
class JSBSimEnv(MultiAgentEnv, ABC):
    """
    Abstract class for JSBSim environments.
    """
    ...
    def __init__(self, config: Dict[str, Any]) -> None:
        ...
        self.agents: List[int] = [
            str(agent.id) for agent in self.get_controlled_agents]
    ...
    def get_specific_agent(self, agent_id: str) -> Agent:
        """
    ...
    def get_evader_agents(self) -> List[Evader]:
        """
    ...
    def get_pursuer_agents(self) -> List[Pursuer]:
        """
    ...
    def spawn_agents(self, num_agents: int, 
        ...
        for i in range(num_agents):
            ...
            sim_interface = SimInterface(aircraft_name='x8',
                sim_freq=self.simulation_config['frequency'],
            ...
            if not is_pursuer:
                agent: Evader = Evader(
                    ...
                    jsbsim_interface=sim_interface,
    ...
    def insert_agent(self, agent: Agent) -> None:
        ...
        """
        self.battlespace.add_agent(agent)
    ...
    def insert_agent(self, agent: Agent,
        ...
        """
        Args: 
            agent (Agent): The agent to insert into the environment
    ...
    def mask_dz_commands(self, agent: Agent, dz_mask: np.ndarray,
                            z_bounds: Dict[str, List[float]],
                            projection_time: float = 3.0) -> np.ndarray:
        """
        Args:
            agent (Agent): The agent to mask the dz commands for
    ...
    def mask_pitch_commands(self, agent: Agent, pitch_mask: np.ndarray,
                            z_bounds: Dict[str, List[float]],
                            projection_time: float = 1.0) -> np.ndarray:
        """
        Args:
            agent (Agent): The agent to mask the pitch commands for
    ...
    def mask_psi_commands(self, agent: Agent, yaw_mask: np.ndarray,
        ...
        """
        Args:
            agent (Agent): The agent to mask the yaw commands for
    ...
    def simulate_single(self, agent: Agent, action: np.ndarray) -> None:
...
class PursuerEvaderEnv(JSBSimEnv):
    """
    ...
    def init_agents(self) -> None:
        ...
        current_agent_id: int = self.spawn_pursuers(
            ...
            agent_id=current_agent_id,
            is_controlled=is_pursuer_controlled,
            evader=self.get_evader_agents()[0]  # spawn pursuers around the first evader
        )
        
    def spawn_pursuers(self, num_pursuers: int, agent_id: str,
                       is_controlled: bool, evader: Evader) -> int:
        """
        Args:
            num_pursuers (int): Number of pursuers to spawn
            agent_id (int): The id of the agent to spawn
            is_controlled (bool): True if the agent is controlled, False otherwise
            evader (Agent): The evader to capture
        Returns:
            agent_id (int): The id of NEXT agent to spawn
        """
        ...
        for i in range(num_pursuers):
            ...
            jsbsim_interface: SimInterface = SimInterface(
                aircraft_name = 'x8',
                sim_freq=self.simulation_config['frequency'],
                init_cond=AircraftIC(
                    state_vector.x, state_vector.y, state_vector.z,
                    state_vector.roll_rad, state_vector.pitch_rad,
                    state_vector.yaw_rad, state_vector.speed))
            radius_bubble: float = self.agent_config['interaction']['bubble_radius']
            capture_radius: float = self.agent_config['interaction']['capture_radius']
            agent: Pursuer = Pursuer(
                state_vector=state_vector,
                jsbsim_interface=jsbsim_interface,
                radius_bubble=radius_bubble,
                agent_id=str(agent_id),
                is_controlled=is_controlled,
                capture_radius=capture_radius
    ...
    def init_action_space(self) -> Dict[str, gym.spaces.Dict]:
        ...
        for agent in self.get_controlled_agents:
            ...
            self.action_spaces[agent.id] = action_space
    ...
    def init_observation_space(self) -> Dict[str, gym.spaces.Dict]:
        ...
        for agent in self.get_controlled_agents:
            num_actions = self.action_spaces[agent.id]["action"].nvec.sum(
            )
            ...
            self.observation_spaces[agent.id] = observation_space
    ...
    def reset(self, *, seed=None, options=None) -> Tuple[Dict[str, Any], Dict[str, Any]]:
        ...
        agent: Agent = self.get_specific_agent(self.current_agent)
        num_actions: int = self.action_spaces[agent.id]["action"].nvec.sum(
        )
