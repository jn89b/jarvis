...
from jarvis.algos.attention import (
    TransformerEncoder    
)
...
class AttentionMasking(ActionMaskingRLModule, PPOTorchRLModule, ValueFunctionAPI):
    """
    An RLModule for PPO with action masking that supports a multi-dimensional action mask.
    This module assumes the discrete action space is MultiDiscrete with three branches (roll, alt, vel)
    and that the environment provides an action mask as a MultiBinary tensor of shape (n_roll, n_alt, n_vel).
    ...
    def setup(self):
        super().setup()
        self.self_state_dim:int = self.model_config["self_state_dim"]
        self.num_actions:int = self.action_space['action'].nvec.sum()
        self.token_dim:int = self.model_config["token_dim"]
        # Reset the observation space to include the action mask.
        self.observation_space = self.observation_space_with_mask
        # Instantiate our custom transformer encoder that performs tokenization.
        self.transformer_encoder = TransformerEncoder(
            obs_space=self.observation_space,
            action_space=self.action_space,
            num_outputs=self.num_actions,
            self_state_dim=self.self_state_dim,# adjust based on your environment
            token_dim=self.token_dim # adjust based on your environment
        )

    @override(PPOTorchRLModule)
    def _forward_inference(self, batch: Dict[str, torch.Tensor], **kwargs) -> Dict[str, torch.Tensor]:
        action_mask, batch = self._preprocess_batch(batch)
        # Use the transformer encoder to process the (tokenized) observations.
        outputs = self.transformer_encoder(batch[Columns.OBS])
        # Get the raw logits from the encoder.
        logits = outputs["logits"]
        # Mask the logits based on the action mask.
        masked_outputs = self._mask_action_logits({Columns.ACTION_DIST_INPUTS: logits}, action_mask)
    ...
    def _forward_train(self, batch: Dict[str, torch.Tensor], **kwargs) -> Dict[str, torch.Tensor]:
        outputs = self.transformer_encoder(batch[Columns.OBS])
        logits = outputs["logits"]
        return self._mask_action_logits({Columns.ACTION_DIST_INPUTS: logits}, batch["action_mask"])
