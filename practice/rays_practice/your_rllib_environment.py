...
class YourEnvironment(MultiAgentEnv):
    def __init__(self, config: EnvContext):
        self.is_use_visualization = False  # config['is_use_visualization']
    ...
    def step(self, action_dict):
        ...
        if 'robot_1_high' in action_dict:
            ...
            self.target_system.robot_1.set_chicken_choice(
                action_dict['robot_1_high'])
            self.target_system.robot_2.set_chicken_choice(
                action_dict['robot_2_high'])
        else:
        ...
        if is_robot_still_active('1'):
            ...
            if yard.timestep > TIMEOUT:
                ...
                rew['robot_1_high'] = self.target_system.get_chicken_reward(
                    self.target_system.robot_1)
        ...
        if is_robot_still_active('2'):
            ...
            if yard.timestep > TIMEOUT:
                ...
                # if it made it the robot is all done
                rew['robot_2_high'] = self.target_system.get_chicken_reward(
                    self.target_system.robot_2)
