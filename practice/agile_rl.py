...
import wandb
import yaml
from pettingzoo.classic import connect_four_v3P
    ...
    def fill_replay_buffer(self, memory, opponent):
        ...
        while len(memory) < memory.memory_size:
            ...
            while not (done or truncation):
                ...
                if opponent_first:
                    p0_action = self.env.action_space(
                        "player_0").sample(p0_action_mask)
                else:
                ...
                if done or truncation:
                    ...
                    if done or truncation:
                        ...
                        memory.save_to_memory_vect_envs(
                            np.concatenate(
                                (p0_state, p1_state,
                                 p0_state_flipped, p1_state_flipped)
                            ),
                            [p0_action, p1_action, 6 - p0_action, 6 - p1_action],
                            [
                                LESSON["rewards"]["lose"],
        ...
        if done:
            ...
            if (agent_three_count + opp_three_count) == 0:
                ...
                reward = (
                    ...
                    + self.lesson["rewards"]["opp_three_in_row"] *
                    opp_three_count
                )
        return reward
...
class Opponent:
    ...
    def update_top(self):
        ...
        board = np.array(self.env.env.board).reshape(
    ...
    def outcome(self, action, player, return_length=False):
        ...
        for c, v in enumerate(self.top):
            draw &= (v == self.num_rows) if c != col else (
                v == (self.num_rows - 1))
        ended |= draw
...
if __name__ == "__main__":
    ...
    for lesson_number in range(1, 5):
        ...
        # check current directory for lesson file
        ...
        if LESSON["buffer_warm_up"]:
            warm_up_opponent = Opponent(
                env, difficulty=LESSON["warm_up_opponent"])
                    ...
                    for idx_step in range(max_steps):
                        ...
                        if done or truncation:
                            ...
                            memory.save_to_memory_vect_envs(
                                ...
                                [p0_action, p1_action, 6 -
                                    p0_action, 6 - p1_action],
                                [
                                    reward,
                                    LESSON["rewards"]["lose"],
                                    reward,
                ...
                for agent in pop:
                    with torch.no_grad():
                        ...
                        for i in range(evo_loop):
                            ...
                            # Create opponent of desired difficulty
                            opponent = Opponent(
                                env, difficulty=LESSON["eval_opponent"])
