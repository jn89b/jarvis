...
prediction_time:float = 6.0  # seconds
dt:float = 0.1
num_prediction_steps = int(prediction_time / dt)  # for dt=0.1, this equals 60
start_idx:int = 21
...
for i in range(num_agents):
    ...
    ukf_estimates = []
    ukf_readings = []
    for j in range(len(input_trajs)):
        center_objects_world[j] = center_objects_world[j].squeeze()
        current_state = center_objects_world[j][i, start_idx, :]
...
for i in range(num_agents):
    ...
    for j in range(len(ukf_traj)):
        ...
        for step in range(0, num_steps, slice_size):
            ...
            slice_mse = np.sum(slice_mse)/gt_slice.shape[0]
            # slice_mse = (slice_mse)
...
# plot the 3D
fig, ax = plt.subplots(1, 1, subplot_kw={'projection': '3''d'})
