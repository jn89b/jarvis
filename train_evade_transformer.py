...
def collate_fn(batch: List[Tuple[int, List[dict]]]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, int]:
    ...
    ego_seq = torch.tensor([step['ego'] for step in sequence], dtype=torch.float32)
    vehicles_seq = torch.tensor([step['vehicles'] for step in sequence], dtype=torch.float32)
    waypoints = torch.tensor([step['future_waypoints'] for step in sequence], dtype=torch.float32)

    # Pad sequences to make them consistent
    padded_ego = ego_seq.unsqueeze(0)  # Add batch dimension (1, seq_len, features)
    padded_vehicles = vehicles_seq.unsqueeze(0)  # (1, seq_len, num_vehicles, features)
...
for epoch in range(start_epoch, epochs):
    ...
    for batch in dataloader:
        ...
        for t in range(data_len):
            ...
            if t % print_every == 0:
                print(f"Epoch {epoch + 1}/{epochs}, Step {t}/{seq_length}, Loss: {loss.item():.4f}")
